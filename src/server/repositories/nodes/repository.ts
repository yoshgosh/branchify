...
export async function findById(d: DBLike, nodeId: string): Promise<Node | null> {
    const rows = await d.select().from(nodes).where(eq(nodes.nodeId, nodeId)).limit(1);
...
export async function findByIds(d: DBLike, nodeIds: string[]): Promise<Node[]> {
    ...
    const rows = await d.select().from(nodes).where(inArray(nodes.nodeId, nodeIds));

    return rows.map((r) => fromStoredNode(StoredNodeSchema.parse(r)));
}

export async function listByGraphId(d: DBLike, graphId: string): Promise<Node[]> {
...
export async function create(d: DBLike, data: NodeInsertInput): Promise<Node> {
    ...
    const [row] = await d.insert(nodes).values(toStoredNodeInsert(values)).returning();
...
export async function remove(d: DBLike, nodeId: string): Promise<Node | null> {
    const [row] = await d.delete(nodes).where(eq(nodes.nodeId, nodeId)).returning();
