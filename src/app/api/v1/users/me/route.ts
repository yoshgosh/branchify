...
import { toUserDto } from '@/shared/api/models';
...
export async function GET(): Promise<NextResponse<GetMeRes>> {
    ...
    const out = await getMe(ctx);
    return NextResponse.json({ user: toUserDto(out.user) }, { status: 200 });
}
...
export async function PATCH(req: NextRequest): Promise<NextResponse<UpdateMeRes>> {
    ...
    const out = await updateMe(ctx, { data: body.data });
    return NextResponse.json({ user: toUserDto(out.user) }, { status: 200 });
}
