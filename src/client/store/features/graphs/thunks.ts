...
export const removeGraphThunk = createAsyncThunk<
    { graph: Graph | null },
    { graphId: string }
>("graphs/remove", async ({ graphId }) => {
    const dto = await graphService.removeGraph({ graphId });
    return { graph: dto.graph ? fromGraphDto(dto.graph) : null };
});

export const generateGraphTitleThunk = createAsyncThunk<
    { graph: Graph },
    { graphId: string }
>("graphs/generateTitle", async ({ graphId }) => {
